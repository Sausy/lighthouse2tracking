...
class VIVEnode : public DarkRoom::Transform {
public:
    VIVEnode(int rate);
...
VIVEnode::VIVEnode(int rate)
    ...
    ROS_INFO(" [VIVE] World offset: [%2.3f , %2.3f, %2.3f] %2.3f", world_offset_[0], world_offset_[1], world_offset_[2],
             world_yaw_);
...
bool VIVEnode::setOriginCB(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res) {
    ...
    ROS_INFO(" [VIVE] New world offset: [%2.3f , %2.3f, %2.3f] %2.3f", world_offset_[0], world_offset_[1],
...
void VIVEnode::Run() {
    ...
    while (ros::ok()) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            Matrix4d pose, rotateX;
            rotateX.setZero();
            rotateX(0,0) = 1; rotateX(1,1) = 0; rotateX(1,2) = 1; rotateX(2,1) = -1; rotateX(2,2) = 0; rotateX(3,3) = 1;
            pose.setIdentity();
            pose(0,0) = tf_matrix[0][0]; pose(0,1) = tf_matrix[0][1]; pose(0,2) = tf_matrix[0][2]; pose(0,3) = tf_matrix[0][3];
            pose(1,0) = tf_matrix[1][0]; pose(1,1) = tf_matrix[1][1]; pose(1,2) = tf_matrix[1][2]; pose(1,3) = tf_matrix[1][3];
            pose(2,0) = tf_matrix[2][0]; pose(2,1) = tf_matrix[2][1]; pose(2,2) = tf_matrix[2][2]; pose(2,3) = tf_matrix[2][3];

            // corrects the lighthouse pose to (z-up,x-right,y-front) to make it compatible with DarkRoom
            pose = pose*rotateX;
            ...
            // It's a controller
            if (dev_type == 2) {
                tf_broadcaster_.sendTransform(tf::StampedTransform(tf, ros::Time::now(), "world",
                                                                   "vive_controller" + std::to_string(controller_count++)));
            }
            // It's a lighthouse
            if (dev_type == 4) {
                tf_broadcaster_.sendTransform(tf::StampedTransform(tf, ros::Time::now(), "world",
                                                                   "lighthouse" + std::to_string(lighthouse_count++)));
            }
